...
脚本会从 `EchoHS/flvx` 的最新 Release 下载 Compose 配置，并拉取 `ghcr.io/echohs/flux-panel-backend` 与 `ghcr.io/echohs/vite-frontend`。两个 GHCR Package 必须设为 Public，服务器无需执行 `docker login`。
